...
``torchrun`` / ``torch.distributed.run`` so ``pi_dex.training_runner`` can use
...
def default_training_argv(extra: Sequence[str] | None = None) -> list[str]:
    ...
    if launcher is None:
        prefix = [sys.executable, "-m", "pi_dex.training_launcher"]
    else:
        prefix = [launcher]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not forwarded:
        raise SystemExit(
            ...
            "--runner pi_dex.training_runner:run -- --mode train ..."
        )
