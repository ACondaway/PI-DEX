...
``[K,36]`` action chunks, so this module:
...
* gathers all windows of one episode as ``[S,K,36]`` / ``[S,D]``
...
from pi_dex.data.sharpa_data import _validate_provenance
...
from pi_dex.data.sharpa_dataset import _command_group
...
class EpisodeNormBatch:
    ...
    def __post_init__(self) -> None:
        ...
        if self.left_actions.ndim != 3 or self.right_actions.ndim != 3:
            raise ValueError("actions: expected [S,K,36] left and right")
    ...
    *,
    ...
    with h5py.File(episode.hdf5_path, "r") as handle:
        ...
        groups = (
            ...
            _command_group(handle, _ACTION_FIELDS[4], MOTOR_JOINT_ORDER),
        )
        sub_state_ok = None
        aligned_length = timeline.time.shape[0]
...
def _gather_joint_actions(
    ...
    motor_angles = _angles(motor)
    left = append_motor_to_arm_hand(left, motor_angles)
    right = append_motor_to_arm_hand(right, motor_angles)
