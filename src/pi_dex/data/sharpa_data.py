...
MOTOR_JOINT_ORDER = tuple(f"motor_j{index}" for index in range(MOTOR_JOINT_DIM))
...
_COMMANDED_JOINT_DIMS = {
    ...
    "action/motor/joint_angle": MOTOR_JOINT_DIM,
}
...
class CommandedJointGroup:
    ...
    Only the five canonical ``action/{left,right}_{arm,hand}/joint_angle`` and
    ``action/motor/joint_angle`` paths are accepted. Arrays are copied onto immutable byte backing after one
...
class BimanualLogicalActionChunk:
    ...
    Args:
        left_actions: Float32 ``[K,D]`` left-hand logical actions, where ``D``
            is 31 for Cartesian wrist targets or 36 for joint targets (29 arm+hand
            plus duplicated motor).
...
def derive_bimanual_logical_action_chunk(
    *,
    ...
    Args:
        ...
        motor: Exact ``action/motor/joint_angle`` group (required for ``JOINT_29D``;
            ignored for ``CARTESIAN_31D`` timestamp alignment only).
    ...
    if validated_spec.action_representation is ActionRepresentation.JOINT_29D:
        _require_group_role(
            motor,
            expected_field="action/motor/joint_angle",
            expected_joint_order=MOTOR_JOINT_ORDER,
        )
    elif motor.field_name != "action/motor/joint_angle":
        raise ValueError(
            f"group role: expected 'action/motor/joint_angle', got {motor.field_name!r}"
        )

    groups = (left_arm, left_hand, right_arm, right_hand, motor)
    ...
    selected_left_arm, selected_left_hand, selected_right_arm, selected_right_hand, selected_motor = selected
    ...
    if validated_spec.action_representation is ActionRepresentation.CARTESIAN_31D:
        ...
        left_arm_hand = derive_joint_actions(
            HandSide.LEFT,
            selected_left_arm.joint_angles,
            selected_left_hand.joint_angles,
            provenance=provenance,
        ...
        right_arm_hand = derive_joint_actions(
            HandSide.RIGHT,
            selected_right_arm.joint_angles,
            selected_right_hand.joint_angles,
            provenance=provenance,
        ...
        motor_angles = selected_motor.joint_angles
        left_actions = append_motor_to_arm_hand(left_arm_hand, motor_angles)
        right_actions = append_motor_to_arm_hand(right_arm_hand, motor_angles)
    if validated_spec.timebase is ActionTimebase.ALIGNED_30_HZ:
...
def derive_joint_actions(
    ...
    *,
    ...
    """Preserve one side's absolute commanded arm+hand joints as ``[K,29]`` actions.
...
def _validate_group_joint_order(
    ...
    *,
    ...
    expected_orders = {
        ...
        "action/motor/joint_angle": MOTOR_JOINT_ORDER,
    }
