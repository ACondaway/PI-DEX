...
* ``left_actions`` / ``right_actions`` — ``[K, 36]`` training targets in ``spec.action_mode``
  (absolute commanded joints with duplicated motor, or scheme-A delta relative to ``state(t0)``)
...
from pi_dex.data.observation_contract import ImageDtypeRange
from pi_dex.data.observation_contract import ImageLayout
from pi_dex.data.observation_contract import MissingPromptPolicy
from pi_dex.data.observation_contract import SharpaObservationContract
from pi_dex.data.sharpa_data import CANONICAL_ALIGNED_TIME_FIELD
...
def _build_sample_index_vectorized(
    ...
    *,
    ...
    from pi_dex.data.norm_compute import episode_valid_start_frames
...
class SyntheticJoint29dDataset:
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        left = rng.normal(size=(self.physical_horizon, 36)).astype(np.float32)
        right = rng.normal(size=(self.physical_horizon, 36)).astype(np.float32)
        return {
            "image": image,
...
def _load_action_chunk(
    ...
    *,
    ...
    return derive_bimanual_logical_action_chunk(
        ...
        motor=_command_group(handle, "action/motor/joint_angle", MOTOR_JOINT_ORDER),
