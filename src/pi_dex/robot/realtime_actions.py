...
Logical joint_29d layout per hand is ``[7 arm | 22 hand | 7 motor]``. Motor is
duplicated on both hands in the model chunk; deployment publishes one motor block
(selected side) to ``/action/motor/joint_angle``.
"""
...
from pi_dex.core.actions import split_joint_logical_action
...
ARM_DIM = 7
HAND_DIM = 22
JOINT_29D_DIM = JOINT_LOGICAL_ACTION_DIM


def split_joint_29d_hand_chunk(
    hand_actions: np.ndarray,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Split ``[E, 36]`` into arm ``[E, 7]``, hand ``[E, 22]``, and motor ``[E, 7]``."""
    ...
    return split_joint_logical_action(array)
...
def policy_result_to_sdk_action_dict(
    ...
    *,
    motor_side: Literal["left", "right"] = "left",
    ...
    suitable for ``NorthDirect._send_actions``. Motor commands default to the
    predicted slice from ``motor_side``; ``motor_positions`` optionally overrides
    every step.
    """
    ...
    if motor_side not in {"left", "right"}:
        raise ValueError(f"motor_side: expected 'left' or 'right', got {motor_side!r}")
    try:
    ...
    left_arm, left_hand, left_motor = split_joint_29d_hand_chunk(left)
    right_arm, right_hand, right_motor = split_joint_29d_hand_chunk(right)
    ...
    if motor_positions is not None:
        ...
        if motor.shape != (left.shape[0], MOTOR_JOINT_DIM):
            raise ValueError(
                f"motor_positions: expected [{left.shape[0]}, {MOTOR_JOINT_DIM}] or "
                f"[{MOTOR_JOINT_DIM}], got {motor.shape}"
            )
        payload[SDK_MOTOR_ACTION] = motor.tolist()
    else:
        selected_motor = left_motor if motor_side == "left" else right_motor
        payload[SDK_MOTOR_ACTION] = selected_motor.tolist()
    return payload
