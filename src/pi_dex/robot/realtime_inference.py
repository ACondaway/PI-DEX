"""Offline helpers for joint_29d policy load / SDK↔OpenPI conversion.

Real-robot deployment lives in ``pi_dex.robot`` (OpenPI Runtime + harobotsDL
NorthZmqEnv). This module keeps checkpoint loading for ``pi-dex-serve`` and
offline convert/infer-once smoke tools.
"""
...
from pi_dex.robot.realtime_actions import sdk_action_pub_keys
...
def load_joint29d_policy(
    *,
    ...
    """Load a training checkpoint as a :class:`~pi_dex.serve.deployment.BimanualPolicyAdapter`."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.mode == "convert-smoke":
        ...
        fake_result = {
            "actions": {
                "left": np.zeros((execution_horizon, spec.logical_action_dim), dtype=np.float32),
                "right": np.zeros((execution_horizon, spec.logical_action_dim), dtype=np.float32),
            }
        }
