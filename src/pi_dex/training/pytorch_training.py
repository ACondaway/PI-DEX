...
``[B, 2 * K, 36]``. Pretrained pi05 checkpoints ship 32D action projections;
callers expand them to 36D before training or deployment. Nonsemantic padding
(when present) is neutralized:
...
* the loss is averaged only over the semantic dimensions selected by the
  action representation (31 for Cartesian wrist actions or 36 for direct
  joint actions including motor).
...
def expand_action_projections_from_pretrained(
    model: nn.Module,
    *,
    source_dim: int = PRETRAINED_MODEL_ACTION_DIM,
    target_dim: int = MODEL_ACTION_DIM,
) -> bool:
    """Expand ``action_in_proj`` / ``action_out_proj`` from a pretrained width.

    Copies existing pretrained weights into the leading semantic columns/rows and
    zero-initializes the new motor block (and any remaining padding). Biases for
    newly added output rows are zero.

    Args:
        model: OpenPI ``PI0Pytorch`` model, optionally DDP-wrapped.
        source_dim: Pretrained checkpoint action projection width (32).
        target_dim: Target PI-DEX model action width (36).

    Returns:
        ``True`` when layers were expanded, ``False`` when already at ``target_dim``.

    Raises:
        TypeError: If action projections are not ``nn.Linear``.
        ValueError: If projection shapes are incompatible with the expansion.
    """
    if source_dim >= target_dim:
        raise ValueError(
            f"expand_action_projections_from_pretrained: source_dim ({source_dim}) "
    ...
    if action_input_projection.in_features == target_dim:
        if action_output_projection.out_features != target_dim:
            raise ValueError(
                "model.action_out_proj.out_features: "
                f"expected {target_dim}, got {action_output_projection.out_features}"
            )
        return False
    if action_input_projection.in_features != source_dim:
        raise ValueError(
            "model.action_in_proj.in_features: "
            f"expected {source_dim} or {target_dim}, got {action_input_projection.in_features}"
        )
    if action_output_projection.out_features != source_dim:
        raise ValueError(
            "model.action_out_proj.out_features: "
            f"expected {source_dim}, got {action_output_projection.out_features}"
        )

    hidden_dim = action_input_projection.out_features
    if action_output_projection.in_features != hidden_dim:
        raise ValueError(
            "model.action_out_proj.in_features: "
            f"expected {hidden_dim}, got {action_output_projection.in_features}"
        )

    device = action_input_projection.weight.device
    dtype = action_input_projection.weight.dtype
    expanded_input_projection = nn.Linear(
        target_dim,
        hidden_dim,
    ...
    expanded_output_projection = nn.Linear(
        hidden_dim,
        target_dim,
    ...
    with torch.no_grad():
        expanded_input_projection.weight.zero_()
        expanded_input_projection.weight[:, :source_dim] = action_input_projection.weight
        if action_input_projection.bias is not None:
            expanded_input_projection.bias.copy_(action_input_projection.bias)

        expanded_output_projection.weight.zero_()
        expanded_output_projection.weight[:source_dim, :] = action_output_projection.weight
        if action_output_projection.bias is not None:
            expanded_output_projection.bias.zero_()
            expanded_output_projection.bias[:source_dim] = action_output_projection.bias

    unwrapped_model.action_in_proj = expanded_input_projection
    unwrapped_model.action_out_proj = expanded_output_projection
    return True
...
def neutralize_openpi_dense_action_io(
    ...
    if validated_spec.logical_action_dim >= MODEL_ACTION_DIM:
        return
...
def validate_model_action_batch(actions: Tensor, spec: BimanualActionSpec) -> None:
    ...
    Args:
        actions: PyTorch float32 tensor with shape ``[B, 2 * K, 36]`` on any
    ...
    if actions.ndim != 3:
        raise ValueError(f"actions.ndim: expected 3 for [B, 2K, 36], got {actions.ndim} with shape {actions.shape}")
...
def neutralize_model_padding(values: Tensor, spec: BimanualActionSpec) -> Tensor:
    ...
    Args:
        values: Floating PyTorch tensor with final dimension 36, on any device.
    ...
    Raises:
        ...
        ValueError: If the final dimension is not 36.
    """
    validated_spec = _validated_spec_copy(spec)
...
def reduce_semantic_action_loss(
    ...
    Args:
        elementwise_loss: Floating tensor with shape ``[..., 36]``. For OpenPI
            pi05 training the full shape is ``[B, 2 * K, 36]``.
        spec: Action contract selecting the 31D Cartesian or 36D joint layout.
    ...
    Raises:
        ...
        ValueError: If it has no elements or its final dimension is not 36.
...
def compute_semantic_flow_matching_loss(
    ...
    *,
    ...
    Args:
        ...
        actions: Float32 tensor with shape ``[B, 2 * K, 36]`` on the model device.
    ...
    def train_step(
        ...
        *,
        ...
        Args:
            ...
            actions: Float32 tensor shaped ``[B, 2 * K, 36]`` on that device.
