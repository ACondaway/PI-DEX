...
from pi_dex.training.checkpoints import CHECKPOINT_METADATA_FILENAME
...
from pi_dex.core.training_contract import OPENPI_MODEL_CONTRACT_KEY
...
def create_pi05_model_config(
    ...
    *,
    ...
    The action projection width is :data:`~pi_dex.core.actions.MODEL_ACTION_DIM`
    (36). Pretrained pi05 checkpoints store 32D projections; callers expand them
    before training or deployment. Only the runtime horizon changes to
    ``2 * physical_horizon``.
    ...
    Returns:
        A validated OpenPI ``Pi0Config`` with ``pi05=True`` and action width 36.
...
def create_bimanual_trained_policy(
    ...
    *,
    ...
    from pi_dex.training.pytorch_training import neutralize_openpi_dense_action_io
    from pi_dex.weights.pi05_weights import materialize_expanded_checkpoint_weights
    ...
    with tempfile.TemporaryDirectory(prefix="pi-dex-checkpoint-") as snapshot_directory_name:
        ...
        materialize_expanded_checkpoint_weights(snapshot_directory / MODEL_WEIGHTS_FILENAME)
...
def configure_bimanual_data(
    ...
    *,
    ...
    Args:
        ...
        model_config: OpenPI ``Pi0Config`` with ``pi05=True``, ``action_dim=36``,
...
def create_pytorch_data_loader_from_dataset(
    ...
    *,
    ...
    Returns:
        ...
        with shape ``[B, 2 * K, 36]``. Action dtype is inherited from the
