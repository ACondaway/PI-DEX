...
from pi_dex.training.checkpoint_manager import build_sample_order
from pi_dex.training.checkpoint_manager import order_sha256
from pi_dex.training.checkpoint_manager import publish_training_checkpoint
from pi_dex.training.checkpoint_manager import resume_training_checkpoint
...
from pi_dex.data.observation_contract import OPENPI_IMAGE_KEYS
...
from pi_dex.training.parameter_manifest import build_parameter_manifests
from pi_dex.training.parameter_manifest import require_full_finetune_manifest
...
def run(context: PytorchTrainingLaunchContext) -> int:
    """Launcher entrypoint: ``pi_dex.training.training_runner:run``."""
...
def _run_validate_data(
    *, dataset: SharpaJoint29dDataset, contract: SharpaObservationContract, args: argparse.Namespace
    ...
    _validate_sample_shapes(
        sample, contract=contract, physical_horizon=dataset.spec.physical_horizon,
        logical_action_dim=dataset.spec.logical_action_dim,
    )
...
def _run_train(
    *,
    ...
    try:
        ...
        from openpi import transforms as openpi_transforms
        from pi_dex.training.distributed import barrier
        from pi_dex.training.distributed import cleanup_process_group
        from pi_dex.training.distributed import device_for_rank
        from pi_dex.training.distributed import init_process_group
...
def _run_train_body(
    *,
    ...
    wandb_run = None
    if is_main_process(rank=rank):
        from pi_dex.training.wandb_run import WANDB_ID_FILENAME
...
def _run_synthetic_smoke(
    *, args: argparse.Namespace, spec: BimanualActionSpec, contract: SharpaObservationContract
    ...
    sample = dataset[0]
    _validate_sample_shapes(
        sample, contract=contract, physical_horizon=spec.physical_horizon,
        logical_action_dim=spec.logical_action_dim,
    )
    payload = {
...
def _validate_sample_shapes(
    ...
    *,
    ...
    expected = (physical_horizon, logical_action_dim)
...
def _parse_runner_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="pi_dex.training.training_runner")
    parser.add_argument(
        "--mode",
        required=True,
