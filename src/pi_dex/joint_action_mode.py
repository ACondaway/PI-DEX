...
def bimanual_joint_reference_from_state(
    ...
    *,
    ...
    """Return left/right ``[29]`` (or ``[N,29]``) reference joints from observation state."""
    ...
    left = np.asarray(array[..., :dim], dtype=np.float32)
    right = np.asarray(array[..., dim : 2 * dim], dtype=np.float32)
