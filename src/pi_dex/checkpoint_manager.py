...
* ``pi_dex.json`` via :func:`pi_dex.checkpoints.save_training_contract`
...
def resume_training_checkpoint(
    *,
    ...
    incompatible = load_model(model, str(weights), strict=True)
    missing = tuple(getattr(incompatible, "missing_keys", ()) or ())
    unexpected = tuple(getattr(incompatible, "unexpected_keys", ()) or ())
    if missing or unexpected:
        raise ValueError(f"resume load_model mismatch missing={missing} unexpected={unexpected}")
